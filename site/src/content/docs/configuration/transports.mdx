...
| **Webhook** (v2.0) | Non-mail sink: deliver the structured submission as a signed JSON POST to a receiver you run. | HMAC signature (+ your own static headers) | JSON |

### Capability matrix (v2.0)

Sending capabilities are uniform; [lifecycle event](/features/lifecycle/) ingestion is not — it's built per provider dialect, Postmark first.

| Capability | Postmark | Resend | Mailgun | SES | SMTP out | Webhook |
|---|---|---|---|---|---|---|
| Text / HTML body | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ (both in payload) |
| Attachments | ✓ | ✓ | ✓ | ✓ | ✓ | — |
| Retry queue eligible | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| Lifecycle events in | ✓ | on demand | on demand | on demand | n/a | n/a |
...
<CardGrid>
  ...
  <LinkCard title="Webhook" href="/configuration/transports/webhook/" description="Signed JSON POST to your own receiver. (v2.0)" />
</CardGrid>
