...
Posthorn enforces this at config-parse time: `auth_required = "none"` on a bind address it can't verify as loopback/private is a startup error unless you set `trusted_network = true` — your explicit assertion that the network the listener sits on is the trust boundary. Inside Docker, binding `:2525` with no `ports:` exposure qualifies; that's why the recipes set it.
</Aside>
...
3. **Write `posthorn.toml`.**
   ...
   auth_required   = "none"
   require_tls     = false
   trusted_network = true             # required with auth "none": asserts the Docker network (no ports: exposure) is the trust boundary
...
| [**Vaultwarden**](/recipes/vaultwarden/) | `SMTP_HOST=posthorn`, `SMTP_PORT=2525`, `SMTP_SECURITY=off` |
