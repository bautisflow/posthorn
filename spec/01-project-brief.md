...
- **2026-07-03** — v1.1 spam-protection scope amended after a full repo audit and operator reports of form-mode spam getting through. Three findings drove three additions. (1) **Blocked-spam visibility** (`on_spam = "discard" | "log"` per endpoint, plus per-check `action = "reject" | "tag"` on the fuzzy checks): every rejection today discards the payload, so operators cannot audit false positives — which is exactly the failure mode the content-shape rules (#34, `max_links = 0` against the observed single-link spam) will create. Aggressive checks without visibility erode trust invisibly; visibility ships in the same release as the checks it audits ([#61](https://github.com/craigmccaskill/posthorn/issues/61)). Digest mode (batched summaries mailed via the endpoint's own transport) is deliberately deferred to v1.1.x — highest operator value but the first time-based behavior in the codebase; it gets its own design pass. (2) **Single-use tokens** ([#62](https://github.com/craigmccaskill/posthorn/issues/62)): CSRF/proof-of-browser tokens are HMAC timestamps with no replay protection, so one harvested token replays botnet-wide for the full TTL — a consumed-token LRU (ADR-8 pattern, in-memory, stateless-v1 compatible) is a prerequisite for the min-age check (#32) and proof-of-browser (#45) being meaningful. (3) **`origin_policy = "strict"`** ([#63](https://github.com/craigmccaskill/posthorn/issues/63)): opt-in close of the accepted v1.0 trade-off where a present-but-mismatched Origin still passes on a matching Referer. Same amendment: milestone 1 formally reshaped by the validated-workload scoring already public on the roadmap — #35 (domain blocklist) and #36 (per-day cap) demoted out (0/5 against the observed corpus), #44/#45 and the pipeline pre-work (#60) admitted. Per-IP strike memory considered and deferred: revisit with data from the new spam metrics rather than speculating. The pipeline architecture decision (ordered binary-verdict checks with per-check action, not a weighted score) is recorded in [#60](https://github.com/craigmccaskill/posthorn/issues/60).
- **2026-08-02** — v2 milestone recut against first principles before implementation began, triggered by external demand on HTML body support ([#5](https://github.com/craigmccaskill/posthorn/issues/5)) and a deliberate re-test of every milestone item against the USP: Posthorn is the **integration seam** between an operator's fleet and the provider they already trust (ingress adaptation, centralization of the duplicated outbound concern, hold-it-in-your-head ops ethos). It is not a lighter Postal and never competes on the delivery-platform axis. Sorted against that bar, the milestone splits cleanly. **In (ships as v2.0):** HTML body (#5, live external demand), SQLite submission log + sync-first retry queue (#21) and durable idempotency (#22) — reliability of the gateway function itself; Postmark lifecycle callbacks (#8) — the abstraction leaks today because callers need provider credentials to learn delivery outcomes, and forwarding provider events is adapter work; the minimal defensive slice of suppression (#4) — auto-suppress on hard bounce/complaint protects the operator's provider account, with CLI-only management; webhook transport (#23) — the seam generalized to a non-mail sink, requiring the ADR-12 amendment (Message gains optional raw `Fields`); file attachments (#24) — passthrough of a message property, form + api-mode ingress only. **Out (demand-gated, recorded below):** unsubscribe injection (#9) — primarily serves bulk-ish sending, pulling toward the marketing lane, and drags `public_url` plus a hosted public endpoint with it; fan-out (#25) — no named operator need. Suppression is named deliberately as the moment Posthorn crosses from stateless gateway to stateful platform; the boundary holding principle 3 is that we store the minimum state that protects the operator's provider account, nothing more. Two doctrine points from the same session: security-relevant features are **opt-in, never default-on** (generalizing ADR-10's philosophy; now NFR25), and lifecycle features require Posthorn to be reachable by the provider's webhooks — a deployment-shape caveat that must be documented plainly because a large slice of the audience runs Posthorn deliberately unreachable from the internet. Milestones are planning artifacts, never commitments; each gets re-evaluated against the design principles before implementation (this entry is the second such recut after 2026-07-03's milestone-1 reshape). New FRs span FR71-FR92, new NFRs NFR25-NFR31, new Epics 13-18, new ADRs 19-25.
...
**v2.0 — gateway reliability + seam features** (recut 2026-08-02 against the USP; see status log). The stateful shift, scoped to what strengthens the integration seam:
- SQLite submission log + sync-first retry queue across restarts (200 still means sent; a new 202 "queued" state covers only what v1.x dropped after exhausting inline retries)
- Durable idempotency (replaces the v1.0 in-memory cache when `[storage]` is configured)
- Postmark lifecycle event callbacks forwarded to the originating caller via HMAC-signed webhook, plus the minimal defensive suppression slice (auto-suppress on hard bounce/spam complaint; CLI management; protects the operator's provider account)
- HTML body support (`body_format = "html"`, contextual auto-escaping, auto-derived text part)
- File attachments (form + api-mode ingress, opt-in, fail-closed type allowlist)
- Webhook transport (the seam generalized to a non-mail sink)

Storage is optional: without a `[storage]` block, v2.0 behaves exactly like v1.x. Lifecycle callbacks additionally require Posthorn to be reachable by the provider's webhooks — deployments that are deliberately unreachable from the internet keep everything else and manage suppressions via CLI only.
...
- **Automatic unsubscribe link injection** (per-recipient signed tokens, hosted unsubscribe endpoint, RFC 8058 one-click headers). Scoped for v2 when the milestone was cut; removed 2026-08-02 during the first-principles recut. Reasoning: the feature primarily serves bulk-ish sending, which pulls toward the marketing lane (principle 3 — Listmonk's territory), and it drags real surface with it: a server-level `public_url` config, a hosted public GET/POST endpoint, and no-expiry tokens in the threat model. The design in [#9](https://github.com/craigmccaskill/posthorn/issues/9) stays valid (suppression is the dependency; the GET must render a confirm page and only the RFC 8058 POST suppresses, so mail scanners can't trigger unsubscribes). Trigger to reconsider: a real bulk-ish send is actually imminent for a named operator. Guardrail regardless of trigger: compliance plumbing only — no scheduling, segments, campaign state, or open/click analytics, ever.
- **Multiple outputs per endpoint (fan-out)** (one submission dispatched to several transports). Scoped for v2 when the milestone was cut; removed 2026-08-02 during the first-principles recut — no named operator need, which is exactly the bar #35/#36 failed. The best-effort semantics sketched in [#25](https://github.com/craigmccaskill/posthorn/issues/25) (200 if at least one output succeeded, per-output failure logging) still look right when a concrete workload surfaces.
- **SMTP-listener attachment passthrough** (extracting attachments from inbound MIME and forwarding them). Deliberately excluded from the v2.0 attachments scope (2026-08-02): no app in any published recipe attaches files to its notifications, and it grows the trickiest parse surface in the codebase (nested multipart) for a user who doesn't exist. Trigger to reconsider: a named internal app that sends attachments via SMTP.
...
- Implement content-based abuse detection or spam classification on relayed mail (the provider's spam systems handle this; Posthorn is a thin relay, not a filter). This posture is about *outbound* classification. The bounded, operator-configured *inbound* form-submission gates (honeypot, origin checks, and the v1.1 spam-protection checks) are protective plumbing for the form-mode threat model — they gate what enters the relay, they don't classify what it forwards, and they don't conflict with this posture.
- Coordinate quota across multiple providers (one provider per endpoint; per-endpoint quotas independent)
- Own, promise, or **test** deliverability, DKIM/SPF/DMARC, or inbox placement. The provider signs (DKIM) and manages reputation; the operator owns the DNS. Posthorn is upstream of signing — it composes a fresh request and hands it off, never touching an already-signed message — so it cannot break DKIM and has nothing to assert about it. This bounds the test strategy (2026-07-03, [#76](https://github.com/craigmccaskill/posthorn/issues/76)): the live-provider tier verifies only that a *well-formed request is accepted by the real API* (simulator/test targets, no delivery). We deliberately do **not** build inbox/DKIM delivery assertions — that would be testing infrastructure this project exists to let operators not run. Content fidelity of the bytes Posthorn does compose (notably the hand-written SMTP-relay MIME) is a separate, in-scope concern, covered by the hermetic ingress→egress harness.
