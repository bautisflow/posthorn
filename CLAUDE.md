...
**Phase: v1.1.0 released 2026-07-04** (GitHub Release + multi-arch GHCR image, cosign-signed, SBOM+provenance attested; validated via `v1.1.0-rc.1` and `make test-live` before GA). `:latest`/`:1`/`:1.1`/`:1.1.0` → the release; `:1.0.0` preserved. v1.0.0 was 2026-05-26; 203 stars / 10 forks as of 2026-07-03; two external humans engaged (amit-tewari #30 multi-tenant SMTP; monperrus built python-posthorn, merged docs PR #48). **v1.1.0 = the full-audit hardening release** (PRs #49/#64/#65/#66/#67/#77/#78/#79 + 6 Dependabot bumps): #37 listener-only-config bug, #42 server timeouts + generic parse errors, #41 auth-none public-bind refusal (BREAKING — needs `trusted_network = true`), #50 SMTP AUTH brute-force + connection caps, #59 shared retry policy, #38/#39/#56 CI/supply-chain, spam observability, the #76 provider-testing harness (hermetic + live, zero-key), plus stack-review fixes. Spec gained FR69/FR70 + ADR-18. Versioned v1.1.0 (minor; #41 break documented prominently — v2 reserved for the platform milestone). Remaining audit queue (post-release): #51–55 (lesser security), #57 (SMTP metrics dead code), #58 (validate skips SMTP checks), #74 (auth-lockout refuse-before-verify), #75 (SMTP terminal→5xx). **Open bookkeeping:** the brief's "operator validation truth" + docs/manual-test.md still list Mailgun/SES/SMTP-relay as unvalidated — update once we confirm which providers `make test-live` actually exercised (which had creds vs skipped). Next feature milestone: **Spam protection** ([milestone 1](https://github.com/craigmccaskill/posthorn/milestone/1) — renamed from "v1.1"; version numbers ≠ milestone names; reshaped 2026-07-03: #31–34, #44, #45, #60 pre-work, #61–63 scope additions; #35/#36 demoted; no code started). The site's [roadmap page](./site/src/content/docs/roadmap.mdx) is the canonical public roadmap.
