...
**Phase:** v1.0.0 shipped. Tagged and released 2026-05-26 (GitHub Release + multi-arch GHCR image). 203 stars / 10 forks as of 2026-07-03; two external humans engaged (amit-tewari #30 multi-tenant SMTP; monperrus built python-posthorn, merged docs PR #48). **2026-07-03/04 full audit → v1.1.0 hardening release, staged for tag.** The audit stack merged to main (PRs #49/#64/#65/#66/#67/#77/#78 + 6 Dependabot bumps): #37 listener-only-config bug, #42 server timeouts + generic parse errors, #41 auth-none public-bind refusal (BREAKING — needs `trusted_network = true`), #50 SMTP AUTH brute-force + connection caps, #59 shared retry policy, #38/#39/#56 CI/supply-chain, spam observability, the #76 provider-testing harness (hermetic + live, zero-key), plus stack-review fixes. Spec gained FR69/FR70 + ADR-18. **Versioning decision (2026-07-04): this ships as v1.1.0** (minor, with the #41 breaking change documented prominently — v2 is reserved for the platform milestone, and the break only closes an insecure config). CHANGELOG staged as [1.1.0]; **release process is RC-first** (`v1.1.0-rc.1` validates the never-run release.yml — provenance/SBOM/cosign/Trivy — without touching `:latest`; then `make test-live` clears the live-validation debt; then `v1.1.0` GA). Not yet tagged. Remaining audit queue: #51–55 (lesser security), #57 (SMTP metrics dead code), #58 (validate skips SMTP checks), #74 (auth-lockout refuse-before-verify), #75 (SMTP terminal→5xx). Next feature milestone: **Spam protection** ([milestone 1](https://github.com/craigmccaskill/posthorn/milestone/1) — renamed from "v1.1"; version numbers ≠ milestone names; reshaped 2026-07-03: #31–34, #44, #45, #60 pre-work, #61–63 scope additions; #35/#36 demoted; no code started). The site's [roadmap page](./site/src/content/docs/roadmap.mdx) is the canonical public roadmap.
